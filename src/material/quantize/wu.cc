...
#include "material/quantize/wu.h"
...
namespace material_color_utilities
{
	...
	{
		return (r << (kIndexBits * 2)) + (r << (kIndexBits + 1)) + (g << kIndexBits) + r +
			   g + b;
	}
	...
	void ComputeMoments(IntArray& weights, IntArray& m_r, IntArray& m_g, IntArray& m_b,
						DoubleArray& moments)
	{
	...
	int64_t Bottom(const Box& cube, const Direction direction, const IntArray& moment)
	{
		if (direction == Direction::kRed)
		{
	...
	double Variance(const Box& cube, const IntArray& weights, const IntArray& m_r,
					const IntArray& m_g, const IntArray& m_b, const DoubleArray& moments)
	{
	...
	double Maximize(const Box& cube, const Direction direction, const int first,
					...
					const int64_t whole_r, const int64_t whole_g, const int64_t whole_b,
					const IntArray& weights, const IntArray& m_r, const IntArray& m_g,
					const IntArray& m_b)
	{
	...
	std::vector<Argb> QuantizeWu(const std::vector<Argb>& pixels, uint16_t max_colors)
	{
