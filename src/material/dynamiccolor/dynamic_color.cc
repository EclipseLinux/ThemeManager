...
#include "material/contrast/contrast.h"
...
namespace material_color_utilities
{
	...
	double ForegroundTone(double bg_tone, double ratio)
	{
		...
		{
			return darker_ratio >= ratio || darker_ratio >= lighter_ratio ? darker_tone
																		  : lighter_tone;
		}
	}
	...
	DynamicColor::DynamicColor(
		...
		std::optional<std::function<ToneDeltaPair(const DynamicScheme&)>> tone_delta_pair)
		: name_(name), palette_(palette), tone_(tone), is_background_(is_background),
		  background_(background), second_background_(second_background),
		  contrast_curve_(contrast_curve), tone_delta_pair_(tone_delta_pair)
	{
	}
	...
	double DynamicColor::GetTone(const DynamicScheme& scheme)
	{
		...
		if (tone_delta_pair_ != std::nullopt)
		{
			...
			bool a_is_nearer = (polarity == TonePolarity::kNearer ||
			...
			double n_contrast = nearer.contrast_curve_.value().get(scheme.contrast_level);
			double f_contrast =
