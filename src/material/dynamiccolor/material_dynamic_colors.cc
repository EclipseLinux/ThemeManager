...
#include "material/dynamiccolor/material_dynamic_colors.h"

#include <cmath>

#include "material/cam/cam.h"
#include "material/cam/hct.h"
#include "material/dislike/dislike.h"
#include "material/dynamiccolor/contrast_curve.h"
#include "material/dynamiccolor/dynamic_color.h"
#include "material/dynamiccolor/dynamic_scheme.h"
#include "material/dynamiccolor/tone_delta_pair.h"
...
namespace material_color_utilities
{
	...
	{
		double alpha =
			(cam.chroma == 0.0 || cam.j == 0.0) ? 0.0 : cam.chroma / sqrt(cam.j / 100.0);

		double t =
			pow(alpha / pow(1.64 - pow(0.29,
		...
		double p1 =
			e_hue * (50000.0 / 13.0) * viewing_conditions.n_c * viewing_conditions.ncb;
	...
	Hct InViewingConditions(Hct hct, ViewingConditions vc)
	{
		...
		Cam recast_in_vc = CamFromXyzAndViewingConditions(
			viewed_in_vc.a, viewed_in_vc.b, viewed_in_vc.c, kDefaultViewingConditions);
	...
	DynamicColor MaterialDynamicColors::SurfaceContainerLowest()
	{
		return DynamicColor(
			...
			/* tone= */
			[](const DynamicScheme& s) -> double {
				return s.is_dark ? ContrastCurve(4.0, 4.0, 2.0, 0.0).get(s.contrast_level)
								 : 100.0;
			},
			/* isBackground= */ true,
			/* background= */ nullopt,
		...
		return DynamicColor(
			...
			[](const DynamicScheme& s) -> double
			{
				return s.is_dark
						   ...
						   : ContrastCurve(96.0, 96.0, 96.0, 95.0).get(s.contrast_level);
			},
			/* isBackground= */ true,
			/* background= */ nullopt,
			/* secondBackground= */ nullopt,
		...
		return DynamicColor(
			...
			[](const DynamicScheme& s) -> double
			{
				return s.is_dark
						   ...
						   : ContrastCurve(94.0, 94.0, 92.0, 90.0).get(s.contrast_level);
			},
			/* isBackground= */ true,
			/* background= */ nullopt,
			/* secondBackground= */ nullopt,
		...
		return DynamicColor(
			...
			[](const DynamicScheme& s) -> double
			{
				return s.is_dark
						   ...
						   : ContrastCurve(92.0, 92.0, 88.0, 85.0).get(s.contrast_level);
			},
			/* isBackground= */ true,
			/* background= */ nullopt,
			/* secondBackground= */ nullopt,
		...
		return DynamicColor(
			...
			[](const DynamicScheme& s) -> double
			{
				return s.is_dark
						   ...
						   : ContrastCurve(90.0, 90.0, 84.0, 80.0).get(s.contrast_level);
			},
			/* isBackground= */ true,
			/* background= */ nullopt,
			/* secondBackground= */ nullopt,
	...
	DynamicColor MaterialDynamicColors::OnSecondaryContainer()
	{
		return DynamicColor(
			...
			/* isBackground= */ false,
			/* background= */
			[](const DynamicScheme& s) -> DynamicColor { return SecondaryContainer(); },
			/* secondBackground= */ nullopt,
			/* contrastCurve= */ ContrastCurve(4.5, 7.0, 11.0, 21.0),
			/* toneDeltaPair= */ nullopt);
	}
	...
	DynamicColor MaterialDynamicColors::OnSecondaryFixed()
	{
		return DynamicColor(
			...
			/* isBackground= */ false,
			/* background= */
			[](const DynamicScheme& s) -> DynamicColor { return SecondaryFixedDim(); },
			/* secondBackground= */
