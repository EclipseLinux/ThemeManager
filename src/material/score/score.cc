...
namespace material_color_utilities
{
	...
	std::vector<Argb> RankedSuggestions(
		const std::map<Argb, uint32_t>& argb_to_population, const ScoreOptions& options)
	{
		...
		for (Hct hct : colors_hct)
		{
			...
			double chroma_weight = hct.get_chroma() < kTargetChroma ? kWeightChromaBelow
																	: kWeightChromaAbove;
		...
		for (int difference_degrees = 90; difference_degrees >= 15; difference_degrees--)
		{
			...
			for (auto entry : scored_hcts)
			{
				...
				if (duplicate_hue == chosen_colors.end())
				{
					...
					if (chosen_colors.size() >= options.desired)
						break;
				}
			}
			if (chosen_colors.size() >= options.desired)
